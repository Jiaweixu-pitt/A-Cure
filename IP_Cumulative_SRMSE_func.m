...
output_folder = strcat(pwd,'\outPut\IP_Cumulative_SRMSE\');
    ...
    for i_noise = 1: length(noise_number)
        for i = 1 : length(severe_ratio)
            ...
            for i_r_ip = 1:size_report_set
                ...
                report_certain_ip = report_certain_ip(result,:);
                report_IP_acc(i_r_ip,1) = i_r_ip;
            ...
            str_RMSE_IP_Cumulative_SRMSE.(name_txt) = RMSE_IP_SRMSE;
...
%save all variables
variableName = strcat(folder0,'Results_All_IP_Cumulative_RMSE.mat');
save(variableName,'str_Acc', 'str_RMSE_Noise', 'str_RMSE_Clean',...
    'str_RMSE_IP_Cumulative_SRMSE');

flag = 'IP_Cumulative_SRMSE method done';
end
